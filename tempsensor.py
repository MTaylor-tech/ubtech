...
GPIO.setup(sensor_pin,GPIO.OUT)  # Set it as an output so that we can:
GPIO.output(sensor_pin,GPIO.HIGH) # write a 1
time.sleep(0.025) # for 25 ms
GPIO.output(sensor_pin,GPIO.LOW) # then write a 0
...
GPIO.setup(sensor_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP) # Change the pin to read mode, with a pullup resistor

for i in range(0,500): # 501 times
    data.append(GPIO.input(sensor_pin)) # read a bit from the GPIO, as fast as possible (no wait)
